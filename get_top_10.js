...
// Sort and display top 20 to ensure we get 10 non-skipped
const sorted = Object.entries(results)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 20);

console.log('Top files with UI emojis needing Lucide icon replacement:\n');
sorted.forEach(([file, count], idx) => {
  const parts = file.split(path.sep);
  const normalizedPath = parts.join('/');
  console.log(String(idx + 1).padStart(2) + '. ' + String(count).padStart(2) + ' emojis - ' + normalizedPath);
});
