...
async function sendChat(req, res) {
  try {
    ...
    // Verify bot belongs to organization and get language setting
    const botCheck = await db.query(
      'SELECT id, language FROM bots WHERE id = $1 AND organization_id = $2',
      [botId, organizationId]
    );
    ...
    const botLanguage = botCheck.rows[0].language;
    ...
    if (botLanguage && botLanguage !== 'en') {
      ...
      const langName = languageNames[botLanguage] || botLanguage;
