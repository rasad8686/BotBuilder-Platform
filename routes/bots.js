...
// CREATE BOT (TOKEN OTOMATİK OLUŞTURULUR)
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    // Token otomatik oluştur
    const token = `bot-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

    const result = await pool.query(
      'INSERT INTO bots (user_id, name, description, token) VALUES ($1, $2, $3, $4) RETURNING *',
      [req.user.userId, name, description, token]
    );
